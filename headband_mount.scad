...
module triangles (){
         for (i = [0:30:360])
                            rotate([0, 0, i+15])
                                translate([-headband_mount_inner_d+0.3, 0, 0])
                                    color("red") circle(0.3, $fn = 3);
    }
    
module outer_shell(radius_off = 0){color("blue") circle(headband_mount_outer_d + radius_off, $fn = 12);}

module inner_shell(){color("green") circle(headband_mount_inner_d + spacing, $fn = 12);}
